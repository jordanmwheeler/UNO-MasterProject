...
AMZN
colnames(AMZN) = c("Open", "High", "Low", "Close", "Volume")
AMZN.Dat = as.data.frame(AMZN)
AMZN.Dat$Date = as.Date(rownames(AMZN.Dat), format = "%Y-%m-%d")
AMZN.Dat$Date.Time = as.POSIXct(rownames(AMZN.Dat), tz = "EST")
AMZN.Dat = AMZN.Dat[AMZN.Dat$Date >= "2019-03-01" & AMZN.Dat$Date <= "2019-03-15",]
AMZN.Dat$PercentChange = AMZN.Dat$Close / Lag(AMZN.Dat$Close, 1) - 1
AMZN.Dat$PercentChange = scales::percent(AMZN.Dat$PercentChange)
AMZN.Dat$Time = format(AMZN.Dat$Date.Time, format="%H:%M:%S")
AMZN.Dat = AMZN.Dat[,c(6,9,1,2,3,4,5,8,7)]
rownames(AMZN.Dat) = NULL
tail(AMZN.Dat)



as.Date(index(stockSymDF), format = "%Y-%m-%d")
as.POSIXct(index(stockSymDF), tz = "EST")
format(index(stockSymDF), format="%H:%M:%S")

stockSym = toupper("amzn")
stockSymDF = getSymbols(stockSym, src = "av", api.key = "ZGYWNP1KJ5A6FALM", output.size = "full", periodicity = "intraday", interval = "1min", env = NULL)
...
stockSymDF = stockSymDF[,c(7,6,8,1,2,3,4,5)]
rownames(stockSymDF) = NULL
stockSymDF.For = stockSymDF
...
numPredictions = 10
numDay = numPredictions + (ceiling(numPredictions/7)*3)
possibleDays = stockSymDF.For[nrow(stockSymDF.For),]$Date.Time + minutes(1:numDay)
possible.weekdays = possibleDays[!weekdays(possibleDays) %in% c("Saturday", "Sunday")]
possible.weekdays = possible.weekdays[1:numPredictions]
...
predictionDF$Forecast = "Forecast"
GARCHDF = data.frame(Date = stockSymDF.For[,c(1)], Close = stockSymDF.For[,c(7)], Lower.Bound = NA, Upper.Bound = NA, Forecast = "Actual")
GARCHDF = rbind(GARCHDF, predictionDF[,c(1,2,4,5,7)])
GARCHDF$Color.Cat = GARCHDF$Forecast
temp.row = GARCHDF[nrow(dplyr::filter(GARCHDF, Color.Cat == "Actual")),]
GARCHDF[nrow(dplyr::filter(GARCHDF, Color.Cat == "Actual")),c("Lower.Bound")] = GARCHDF[nrow(dplyr::filter(GARCHDF, Color.Cat == "Actual")),c("Close")]
