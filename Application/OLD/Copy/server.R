...
function(input, output, session) {
  ...
  output$intra.ov.overviewPlot <- renderPlotly({
    ...
    stockSymDF = getSymbols(stockSym, src = "av", api.key = "ZGYWNP1KJ5A6FALM", output.size = "full", periodicity = "intraday", interval = input$intra.ov.intraday.step, env = NULL)
  ...
  intra.GARCHDF <- eventReactive(input$intra.forecastButton, {
    withProgress(message = 'Running GRACH Forecasting', 
                 ...
                 value = 0,
                 {
                   ...
                   intra.stockSymDF.For = getSymbols(intra.stockSym.For, src = "av", api.key = "ZGYWNP1KJ5A6FALM", output.size = "full", periodicity = "intraday", interval = input$intra.intraday.step, env = NULL)
                   ...
                   possibleDays = intra.stockSymDF.For[nrow(intra.stockSymDF.For),]$Date.Time + minutes(1:numDay)
                   ...
                   GARCHfit = garchAuto(intra.stockSymDF.For$Return, trace=TRUE)
                   ...
                   for(i in 1:numPredictions){
                     if(i == 1){
                       intra.predictionDF[i,2] = exp(as.numeric(log(intra.stockSymDF.For[nrow(intra.stockSymDF.For),]$Close)) + as.numeric(intra.predictionDF[i,3]))
                       intra.predictionDF[i,4] = exp(as.numeric(log(intra.stockSymDF.For[nrow(intra.stockSymDF.For),]$Close)) - as.numeric(intra.predictionDF[i,3]+(1.96*intra.predictionDF[i,6])))
                       intra.predictionDF[i,5] = exp(as.numeric(log(intra.stockSymDF.For[nrow(intra.stockSymDF.For),]$Close)) + as.numeric(intra.predictionDF[i,3]+(1.96*intra.predictionDF[i,6])))
                     }
                     else{
                       intra.predictionDF[i,2] = exp(as.numeric(log(intra.predictionDF[i-1,2])) + as.numeric(intra.predictionDF[i,3]))
                       intra.predictionDF[i,4] = exp(as.numeric(log(intra.predictionDF[i-1,2])) - as.numeric(intra.predictionDF[i,3]+(1.96*intra.predictionDF[i,6])))
                       intra.predictionDF[i,5] = exp(as.numeric(log(intra.predictionDF[i-1,2])) + as.numeric(intra.predictionDF[i,3]+(1.96*intra.predictionDF[i,6])))
                     }
                   }
