...
function(input, output, session) {
  ...
  GARCHDF = reactiveValues(data = NULL)
  ...
  #                  GARCHDF2 = data.frame(Date = stockSymDF.For[,c(1)], Close = stockSymDF.For[,c(5)], Lower.Bound = NA, Upper.Bound = NA, Forecast = "Actual")
  #                  GARCHDF2 = rbind(GARCHDF2, predictionDF[,c(1,2,4,5,7)])
  #                  GARCHDF2$Color.Cat = GARCHDF2$Forecast
  #                  temp.row = GARCHDF2[nrow(dplyr::filter(GARCHDF2, Color.Cat == "Actual")),]
  #                  GARCHDF2[nrow(dplyr::filter(GARCHDF2, Color.Cat == "Actual")),c("Color.Cat")] = "Forecast"
  #                  GARCHDF2 = rbind(GARCHDF2, temp.row)
  #                  GARCHDF2$Color.Cat = factor(GARCHDF2$Color.Cat)
  #                  GARCHDF2 = GARCHDF2[order(GARCHDF2$Date, GARCHDF2$Color.Cat),]
  #                  
  #                  incProgress(1/3)
  #                })
  #   
  #   GARCHDF$data <- GARCHDF2
  #   
  # })
  ...
  output$forecastPlot <- renderPlotly({
    if (length(GARCHDF.plot) == 0) {return(NULL)}
    
    # GARCHDF$Date = as.Date(GARCHDF$Date.Char, format = "%Y-%m-%d")
    input$forecastButton
    isolate({
      withProgress(message = 'Running GRACH Forecasting', 
                   detail = "This may take a few minutes...", 
                   value = 0,
      ...
      GARCHDF.plot <- GARCHDF2
    
    plot_ly(GARCHDF.plot, x = ~Date, y = round(GARCHDF.plot$Close, 2), type = 'scatter', mode = 'lines',
            color = ~Color.Cat, colors = c("#377EB8", "#39ac39"),
            hoverinfo = "text",
            hovertext = paste0(GARCHDF.plot$Forecast, "\n", 
                               weekdays(GARCHDF.plot$Date), ": ", GARCHDF.plot$Date, "\n", 
                               "Price: ", scales::dollar(GARCHDF.plot$Close))) %>%
      add_trace(y = round(GARCHDF.plot$Upper.Bound, 2), type = 'scatter', mode = 'lines',
                ...
                hovertext = paste0("Upper Bound Error\n", 
                                   weekdays(GARCHDF.plot$Date), ": ", GARCHDF.plot$Date, "\n", 
                                   "Price: ", scales::dollar(GARCHDF.plot$Upper.Bound))) %>%
      add_trace(y = round(GARCHDF.plot$Lower.Bound, 2), type = 'scatter', mode = 'lines',
                ...
                hovertext = paste0("Lower Bound Error\n", 
                                   weekdays(GARCHDF.plot$Date), ": ", GARCHDF.plot$Date, "\n", 
                                   "Price: ", scales::dollar(GARCHDF.plot$Lower.Bound))) %>%
      layout(title = "Time Series Chart",
             font = list(color = "#ffffff"),
  ...
  # observeEvent(input$man.forecast, {
  ...
  # output$forecastPlot <- renderPlotly({
  #   if (!exists("GARCHDF.plot")) return()
  #   
  #   
  #   GARCHDF.plot = GARCHDF$data
  #   # GARCHDF$Date = as.Date(GARCHDF$Date.Char, format = "%Y-%m-%d")
  #   
  #   plot_ly(GARCHDF.plot, x = ~Date, y = round(GARCHDF.plot$Close, 2), type = 'scatter', mode = 'lines',
  #           color = ~Color.Cat, colors = c("#377EB8", "#39ac39"),
  ...
  output$forecastTable <- DT::renderDataTable({
    if (is.null(GARCHDF$data)) return()
    
    GARCHDF = GARCHDF$data
    GARCHDF.tab <- GARCHDF
    GARCHDF.tab2 <- GARCHDF
    ...
    #   GARCHDF.tab2$Date = paste0(weekdays(GARCHDF.tab2$Date), ": ", GARCHDF.tab2$Date)
    #   GARCHDF.tab2$Close = scales::dollar(GARCHDF.tab2$Close)
    #   GARCHDF.tab2$Lower.Bound = scales::dollar(GARCHDF.tab2$Lower.Bound)
    #   GARCHDF.tab2$Upper.Bound = scales::dollar(GARCHDF.tab2$Upper.Bound)
    #   GARCHDF.tab2 = filter(GARCHDF.tab2, GARCHDF.tab2$Forecast == "Forecast")
    #   DT::datatable(GARCHDF.tab2[,c(1,2,3,4,5)], rownames = FALSE, colnames = c("Date", "Close", "Lower Bound", "Upper Bound", "Actual/Forecast"))  %>% 
    #     DT::formatStyle(columns = c("Date", "Close", "Lower.Bound", "Upper.Bound", "Forecast"), color = '#377EB8', backgroundColor = '#222222', fontWeight = 'bold')
    # }
  })
}
###  %>% DT::formatStyle(colnames(stockSymDF),color = '#377EB8', backgroundColor = '#222222', fontWeight = 'bold')
